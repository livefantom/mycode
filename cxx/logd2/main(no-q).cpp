...
*/// 这部分算法有问题！！！
	...
	printf("\n----initialize maxsz:%d, head=%d, tail=%d", gp_enter_shm->maxsz,
...
int queue_size()
{
    return (gp_enter_shm->tail >= gp_enter_shm->head)
    	? (gp_enter_shm->tail - gp_enter_shm->head)
    	: (gp_enter_shm->maxsz - gp_enter_shm->head + gp_enter_shm->tail);
}

int queue_spare()
{
    return (gp_enter_shm->tail >= gp_enter_shm->head)
		? (gp_enter_shm->maxsz - gp_enter_shm->tail + gp_enter_shm->head)
		: (gp_enter_shm->head - gp_enter_shm->tail);
}
...
{
	int len;
	printf("\n----pre-append--- maxsz:%d, head=%d, tail=%d", gp_enter_shm->maxsz,
		gp_enter_shm->head, gp_enter_shm->tail);

	if ( queue_spare() > 0 )
	{
		if ( gp_enter_shm->tail == gp_enter_shm->maxsz )
			gp_enter_shm->tail = 0;

		memcpy(&gp_enter_shm->data + gp_enter_shm->tail * sizeof(EnterMsg),
				msg, sizeof(EnterMsg));
		++gp_enter_shm->tail;
	}
	else
...
*/
	printf("\n--pre-flush msg----------------head:%d, tail:%d--",
		gp_enter_shm->head, gp_enter_shm->tail);

	// write to database.
	if ( gp_enter_shm->head + FLUSH_ROW <= gp_enter_shm->maxsz )
	{
		gp_enter_shm->head += FLUSH_ROW;
	}
	else
	{
		gp_enter_shm->head = gp_enter_shm->head + FLUSH_ROW - gp_enter_shm->maxsz;
	}
